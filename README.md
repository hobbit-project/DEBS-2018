...
4. Build docker image using the `buildImages()` from the [SampleSystemTest.java](https://github.com/hobbit-project/DEBS-GC-2018/blob/master/src/test/java/org/hobbit/debs_2018_gc_samples/SampleSystemTest.java). The full URI of build image will be shown in console and should look like this `<yourProjectURI>/systemAdapter`. Don't forget to package your codes by the `mvn package -DskipTests=true` before building/rebuilding the image.
